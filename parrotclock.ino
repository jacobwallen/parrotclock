...
void heatHornAndSiren()
{
  if (!pause) {
    ...
    else if (parrotMode && timer.get_time() < heatTime - 30000 && timer.get_time() > heatTime - 30500) {
      digitalWrite(horn, HIGH);
...
void prepHornAndSiren()
{
  ...
  else if (timer.get_time() < heatTime - 400 && timer.get_time() > heatTime - 600) {
    digitalWrite(horn, HIGH);
  }
  else if (timer.get_time() < heatTime - 800 && timer.get_time() > heatTime - 1000) {
    digitalWrite(horn, HIGH);
  }
  // 30 seconds left horn, sound the alarm 0.5s, pause 0.5s and sound 0.5s
  else if (timer.get_time() < thirtysecs && timer.get_time() > thirtysecs - 500) {
    digitalWrite(horn, HIGH);
  }
  else if (timer.get_time() < thirtysecs - 1000 && timer.get_time() > thirtysecs - 1500) {
    digitalWrite(horn, HIGH);
  }
  else {
    digitalWrite(horn, LOW);
